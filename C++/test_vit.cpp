...
public:
    ViTFeatureExtractor() : device_(torch::kCPU) {}
...
cv::Mat crop_and_resize(const cv::Mat& frame, const cv::Rect& box, cv::Size size = cv::Size(224, 224)) {
...
int main() {
    ...
    ViTFeatureExtractor vit;
    if (!vit.load_model(MODEL_PATH)) {
        std::cerr << "无法加载 ViT 模型" << std::endl;
        return -1;
    }
    ...
    while (true) {
        ...
        if (largest_face != nullptr) {
            cv::Mat face = crop_and_resize(frame, largest_face->box);
            face_image = face.clone();
            
            current_tensor = vit.extract_feature(face);
